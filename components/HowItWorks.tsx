          ...
          <ol className="grid md:grid-cols-3 gap-10 md:gap-8 relative">
            {HOW_IT_WORKS.map((step) => (
              <li key={step.step} className="relative flex flex-col items-start">
                <div className="w-12 h-12 rounded-full bg-[color:var(--color-primary)] text-white flex items-center justify-center font-mono text-[13px] font-semibold relative z-10">
                  {step.step}
                </div>
                <h3
                  className="mt-6 text-[22px] leading-[1.2] tracking-[-0.01em] text-[color:var(--color-foreground)]"
                  style={{ fontFamily: "var(--font-display)" }}
                >
                  {step.title}
                </h3>
                <p className="mt-2.5 text-[14.5px] leading-relaxed text-[color:var(--color-foreground-secondary)]">
                  {step.body}
                </p>
              </li>
            ))}
